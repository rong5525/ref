...
def generate_pdf_list_html(subfolder_path):
    # Create bullet points for each PDF
    html_content = '<ul style="list-style-type: disc; padding-left: 40px;">\n'
    for item in sorted(os.listdir(subfolder_path)):
        if item.lower().endswith('.pdf'):
            file_name_without_extension = item.rsplit('.', 1)[0]
            # Create a list item for each PDF
...
def generate_subfolder_index_html(subfolder_path, root_folder):
    ...
    # Ensure the subpage style matches the root page
...
<head>
    ...
    <style>
        body {{
            ...
            font-size: 22px; /* Uniform font size */
            background-color: #f7f8fa;
            color: #333;
            padding: 20px;
        }}
        ul {{
            padding-left: 40px; /* Adjust padding for bullet points */
        }}
        a {{
        ...
        a.return {{
            font-weight: bold;
            text-decoration: none; /* Remove underline from 'Ref' link */
        }}
    </style>
</head>
<body>
    ...
    for first_level_folder in sorted(os.listdir(root_folder)):
        if os.path.isdir(os.path.join(root_folder, first_level_folder)) and first_level_folder != ".git":
            ...
            for second_level_folder in subfolders:
                ...
                generate_subfolder_index_html(second_level_path, root_folder)  # Call the function to generate index for each subfolder
