...
<head>
    ...
    <style>
        body {{
            font-family: Arial, sans-serif; /* Ensure uniform font family */
            font-size: 20px; /* Ensure uniform font size */
...
<head>
    ...
    <style>
        body {{
            font-family: Arial, sans-serif; /* Ensure uniform font family */
            font-size: 22px; /* Ensure uniform font size */
            background-color: #f7f8fa;
            color: #333;
            padding: 20px;
            margin-left: 50px;
...
<body>
    ...
    for first_level_folder in sorted(os.listdir(root_folder)):
        if os.path.isdir(os.path.join(root_folder, first_level_folder)) and first_level_folder != ".git":
            ...
            html_str += ' - '.join(subfolder_links) + '</div>\n'
    html_str += "</body>\n</html>"
